...
class Cupon_models extends CI_Model {
    ...
    public function validar($id) {
        ...
        if(isset($id)) {
            $date=date('Y-m-d');
            $this->db->select('a.descuento,a.cupon_validar');
            $this->db->from('bd_cupon as a');
            $this->db->where('a.status','1');
            $this->db->where('a.cupon_validar',$id);
            $this->db->where('a.fecha_inicio<=',$date);
            $this->db->where('a.fecha_fin>=',$date);
            $query=$this->db->get();
            if($query->num_rows() > 0) {
                foreach($query->result() as $row ) {
                     $data[]=$row;
